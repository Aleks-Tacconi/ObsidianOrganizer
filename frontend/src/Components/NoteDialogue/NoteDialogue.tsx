...
export default function NoteDialog({ open, onClose, primaryTagId, note }: Props) {
    const [name, setName] = useState(note?.name ?? "");
    const [description, setDescription] = useState(note?.description ?? "");
    const [completed, setCompleted] = useState(note?.completed ?? false);

    const [date, setDate] = useState(note ? new Date(note.date).toISOString().slice(0, 16) : new Date().toISOString().slice(0, 16));

    const [subtags, setSubtags] = useState<SubTag[]>([]);
    const [selectedSubtags, setSelectedSubtags] = useState<SubTag[]>(note?.subtags ?? []);

    const [urls, setUrls] = useState<NewNoteURL[]>(note ? note.urls.map((u) => ({ alias: u.alias, url: u.url })) : []);
    ...
    return (
        <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
            ...
            <DialogContent sx={{ display: "flex", flexDirection: "column", gap: 2, mt: 1 }}>
                ...
                <TextField label="Date & Time" type="datetime-local" value={date} onChange={(e) => setDate(e.target.value)} InputLabelProps={{ shrink: true }} />
